...
## [1.4.0](https://github.com/uktrade/django-log-formatter-asim/compare/1.3.1...1.4.0) (2026-08-24)

Thank you to @JasperWagen for their first contribution.

### Fixed

* The `SrcIpAddr` field generated by `ASIMRequestFormatter` was populated with the address of the server,
  e.g. `127.0.0.1`.
  This is now populated with the more expected client IP address.

### Added

* Support for Django running in ASGI mode.
  Versions prior to this cause the `ASIMRequestFormatter` to generate the below error
  ```
